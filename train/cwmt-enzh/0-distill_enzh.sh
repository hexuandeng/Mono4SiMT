...
source ./data_path.sh
TASK=teacher_cwmt
CHECKDIR=/root/Mono4SiM/train/cwmt-${SRC}${TGT}/checkpoints/${TASK}_${SRC}${TGT}
AVG=true
...
if [[ $AVG == "true" ]]; then
  ...
  python /root/Mono4SiM/utility/simultaneous_translation/scripts/average_checkpoints.py \
...
FILES=/root/Mono4SiM/data/cwmt-enzh
mkdir -p ${ROOT}
mkdir -p ${ROOT}/interactive
mkdir -p ${FILES}/split

split -l $((`wc -l < ${FILES}/ready/train.clean.en`/8)) -d -a 1 ${FILES}/ready/train.clean.en ${FILES}/split/train.en.

for i in {0..7}
do
  cat ${FILES}/split/train.en.${i} | CUDA_VISIBLE_DEVICES=${i} \
    python -m fairseq_cli.interactive /root/Mono4SiM/data/cwmt-enzh/data-bin \
    -s ${SRC} -t ${TGT} \
    --user-dir ${USERDIR} \
